...
class CodeLoopbackClient {
    ...
    async listenForAuthCode(
        ...
        const authCodeListener = new Promise<ServerAuthorizationCodeResponse & { url: string }>(
            (resolve, reject) => {
    ...
    static getDeserializedQueryString(query: string): ServerAuthorizationCodeResponse {
        ...
        const deserializedQueryString: ServerAuthorizationCodeResponse = this.queryStringToObject(
...
export class OidcClient {
    ...
    private async _handleCallback(uri: Uri): Promise<TokenInformation | undefined> {
        ...
        try {
            ...
            const { access_token, refresh_token } = json as {
                access_token: string;
                refresh_token: string;
            };
            if (!access_token) {
