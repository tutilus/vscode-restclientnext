...
suite('Extension Test Suite', () => {
    ...
    test('File variable parsing should handle quoted values, descriptions, and comments', async () => {
        const {
            FileVariableProvider,
        } = require('../utils/httpVariableProviders/fileVariableProvider');
        ...
        const testCases: Array<{
            ...
            {
                input: '"abcd | desc" | description',
                expectedValue: 'abcd | desc',
            ...
            {
                input: 'abcd | description # comment',
                expectedValue: 'abcd',
                expectedDesc: 'description',
                description: 'Comment after description',
            },
            {
                input: '"abcd" # comment',
                expectedValue: 'abcd',
                description: 'Comment after quoted value',
            },
            {
                input: '"abcd | desc" # comment',
                expectedValue: 'abcd | desc',
                description: 'Comment after value with pipe',
            },
            {
                input: '"abcd" | description # comment',
                expectedValue: 'abcd',
            ...
            {
                input: '"abcd # not a comment" | description',
                expectedValue: 'abcd # not a comment',
                expectedDesc: 'description',
            },
        ];
        ...
        for (const { input, expectedValue, expectedDesc, description } of testCases) {
            ...
            assert.strictEqual(
                result.value,
                expectedValue,
                `Value mismatch for input: "${input}"${description ? ` (${description})` : ''}`
            );
            if (expectedDesc !== undefined) {
                assert.strictEqual(
                    result.description,
                    expectedDesc,
                    `Description mismatch for input: "${input}"${description ? ` (${description})` : ''}`
                );
            } else {
                assert.strictEqual(
                    result.description,
                    undefined,
                    `Expected no description for input: "${input}"`
                );
            }
        }
    });
